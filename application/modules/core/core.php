...
class Core extends MY_Controller {
    ...
    function _display_page_and_cat($page = array(), $category = array()) {
		...
		$page = $this->_add_gallery_to_page($page);
	...
	function _add_gallery_to_pages($pages = array()) {
		
		$cnt = count($pages);
		for ($i = 0; $i < $cnt; $i++) {
			$pages[$i] = $this->_add_gallery_to_page($pages[$i]);            
        }
		
		return $pages;
	}
	
	function _add_gallery_to_page($page) {
		
		preg_match_all("|\[gallery([0-9]+)\]|Uis",$page['prev_text'],$albums);
		for ($x = 0; $x <= count($albums); $x++) {
			$album = $this->gallery->thumbnails($albums[1][$x],TRUE);
			$page['prev_text'] = str_replace($albums[0][$x],$album,$page['prev_text']);
		}
			
		preg_match_all("|\[gallery([0-9]+)\]|Uis",$page['full_text'],$albums);
		for ($x = 0; $x <= count($albums); $x++) {
			$album = $this->gallery->thumbnails($albums[1][$x],TRUE);
			$page['full_text'] = str_replace($albums[0][$x],$album,$page['full_text']);
		}
    ...
    function _display_category($category = array()) {
        ...
        if ($cnt > 0) {
			...
			$pages = $this->_add_gallery_to_pages($pages);
