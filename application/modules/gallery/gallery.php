...
class Gallery extends MY_Controller {
    ...
    function index() {        
		...
		$albums = $this->add_albums_images($albums);
    ...
    function category($id = 0) {
        $category = $this->gallery_m->get_category($id);
		 ...
		 $this->core->set_meta_tags(array($category['name']));
    ...
    function thumbnails($id = 0, $fetch = FALSE) {
        $album = $this->gallery_m->get_album($id);
		...
		if ($fetch == TRUE) {
			return $this->fetch_tpl('page_album');
		}
	...
	function add_albums_images($albums = array()) {
		$cnt = count($albums);
		for ($i = 0; $i < $cnt; $i++) {
            $albums[$i]['images'] = $this->gallery_m->get_album_images($albums[$i]['id']);
			$albums[$i]['thumb_url'] = $this->conf['upload_url'] . $albums[$i]['id'] . '/' . $this->conf['thumbs_folder'] . '/';
			$albums[$i]['album_url'] = $this->conf['upload_url'] . $albums[$i]['id'] . '/';
        }
