...
class Gallery_m extends CI_Model {
     ...
     * Get Album info
     */
    function get_album($id = 0, $include_images = TRUE, $limit, $page) {
        $this->db->limit(1);
        $this->db->where('id', $id);
        $query = $this->db->get('gallery_albums');
        ...
        if ($query->num_rows() > 0) {
            ...
            if ($include_images == TRUE) {
                $album['images'] = $this->get_album_images($album['id'], $limit, $page);
    ...
    function get_album_images($album_id, $limit, $ofset) {
